...
const Popover = React.forwardRef<HTMLDivElement, PopoverProps>(function PopoverFn(
	...
	useEffect(() => {
		...
		if (activateOnHover && anchorElement) {
			...
			window?.document.addEventListener('scroll', closePopover);
		}
		return (): void => {
			...
			window?.document.removeEventListener('scroll', closePopover);
		};
	}, [anchorEl, activateOnHover, onMouseEnter, onMouseLeave, closePopover, window?.document]);
