...
const ContainerEl = styled.div<ContainerElProps>`
	display: flex;
	flex-direction: ${({ orientation }): any => orientation};
	align-items: ${({ crossAlignment }): any => crossAlignment};
	justify-content: ${({ mainAlignment }): any => mainAlignment};
	flex-wrap: ${({ wrap }): any => wrap};
	flex-grow: ${({ flexGrow }): any => flexGrow};
	flex-shrink: ${({ flexShrink }): any => flexShrink};
	...
	${({ margin }): any =>
		...
		css`
			...
			`};
		`};
	border-radius: ${({ borderRadius, theme }): any => {
		switch (borderRadius) {
			case 'regular':
	...
	padding: ${({ theme, padding }): any => padding && getPadding(padding, theme)};
	gap: ${({ gap }): any => gap};
	&::-webkit-scrollbar {
		width: 0.5rem;
	}
